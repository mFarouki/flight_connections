...
def get_airport_codes_by_country(country_links: dict):
    ...
    for country, link in country_links.items():
        ...
        paragraphs = country_page_soup.find_all('p', class_=KEY_PARAGRAPH_CLASS)
        for paragraph in paragraphs:
            paragraph_text = paragraph.get_text().split(' ')
            city, code = " ".join(paragraph_text[:-1]), paragraph_text[-1].lower()
...
def get_airports_by_country():
    ...
    airports_by_country = get_airport_codes_by_country(country_links)
